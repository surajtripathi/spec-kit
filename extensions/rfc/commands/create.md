---
description: "Create a technical RFC from an existing feature spec, proposing HOW to build it."
handoffs:
...
The text the user typed after the command provides additional context or constraints for the RFC (e.g., preferred technology, architectural direction). Use it to guide decisions but derive the RFC content from the spec.
...
3. **Load RFC template**: The template has been copied to RFC_FILE by the setup script. Read it to understand required sections.
